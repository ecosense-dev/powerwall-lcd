...
static void after_web_save_task(void *arg)
{
    ...
    if (!app_config_needs_wizard() && app_wifi_is_connected()) {
...
static void connect_from_web_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "web wifi connect: %s", esp_err_to_name(err));
    ui_lock();
    ui_settings_reload();
    ui_refresh();
    if (err == ESP_OK) {
        ui_show_wizard(false);
        ui_settings_show_message("WiFi connesso", false);
    } else {
        ui_settings_show_message(app_wifi_last_error(), true);
    }
    ui_unlock();
    vTaskDelete(NULL);
}
...
static char *build_page(const char *flash)
{
    app_config_t cfg;
    app_config_get(&cfg);
    const char *ip = app_wifi_portal_ip();
    if (!ip[0]) {
        ip = APP_WIFI_AP_IP;
    }
    ...
    if (app_wifi_is_connected()) {
        snprintf(line, sizeof(line), "<p class=\"ok\">Connesso a %s &middot; IP %s</p>",
        ...
        snprintf(line, sizeof(line), "<p class=\"err\">WiFi offline</p>");
    }
    append(&html, &len, &cap, line);
    ...
    append(&html, &len, &cap,
           ...
           "<p>Il pannello vede solo reti 2.4 GHz. Dopo il WiFi, apri di nuovo questa pagina "
           "dalla rete di casa per il token.</p></body></html>");
    return html;
}
...
static esp_err_t on_post_save(httpd_req_t *req)
{
    ...
    xTaskCreate(after_web_save_task, "web_save", 4096, NULL, 4, NULL);
    return send_page(req, err == ESP_OK ? "Token salvato." : "Errore NVS.");
}
...
static esp_err_t on_post_wifi(httpd_req_t *req)
{
    ...
    app_config_save();
    xTaskCreate(connect_from_web_task, "web_wifi", 8192, NULL, 5, NULL);
    ...
    const char *msg =
        ...
        "<p>Se il telefono perde la rete, torna sul WiFi di casa e apri l'IP mostrato sul display.</p>"
        "</body></html>";
...
esp_err_t app_httpd_start(void)
{
    ...
    register_uris(s_srv);
    ESP_LOGI(TAG, "HTTP :80 su http://%s/", app_wifi_portal_ip()[0] ? app_wifi_portal_ip() : "0.0.0.0");
    httpd_unlock();
    return ESP_OK;
}
